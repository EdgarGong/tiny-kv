...
type PeerStorage struct {
	...
	// RegionLocalState and RaftApplyState. You can regard kvdb
...
func NewPeerStorage(engines *engine_util.Engines, region *metapb.Region, regionSched chan<- worker.Task, tag string) (*PeerStorage, error) {
	...
	if raftState.LastIndex < applyState.AppliedIndex { // raftState.LastIndex >= applyState.AppliedIndex
...
// Also, update the peer storage’s RaftLocalState and save it to raftdb.
//
// To save the hard state is also very easy,
// just update peer storage’s RaftLocalState.HardState and save it to raftdb
//
// Hints:
